  ...
  return (
    <aside className="w-64 hidden md:flex flex-col bg-white border-r border-gray-200 min-h-screen p-6">
      ...
      <nav className="flex-1 flex flex-col gap-1">
        {items.map((it) => (
          <button key={it.key} onClick={() => setActive(it.key)} className={"w-full px-3 py-2 bg-yellow-200/50 rounded text-left " + (active === it.key ? "font-semibold bg-yellow-300" : "hover:bg-yellow-100")}>
            {it.label}
          </button>
        ))}
      </nav>
